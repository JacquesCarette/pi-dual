...
monoidal categories~\cite{XXX} which are used to model many
resource-aware (e.g., based on linear types) programming
languages~\cite{XXX}
...
memory list by the size of $A$. As an illustrative example, we include
a trace of the execution of \AgdaFunction{id'} from Sec.~\ref{sub:id}
applied to 𝕋:
...
value it expects. A particularly intringuing instance of that
...
\includegraphics[scale=0.5]{trace3.png}
...
thinking of gc as space transformers

\PPFTname{}
\PPFThof{}

trace about gc'ing cycles
...
Yokoyama, T., Axelsen, H.B., Glück, R.: Principles of a reversible
programming language. In: Conference on Computing Frontiers, CF,
pp. 43–54. ACM Press (2008)
