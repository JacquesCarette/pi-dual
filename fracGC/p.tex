...
\assoclt :&  \tau_1 * (\tau_2 * \tau_3) & \iso & (\tau_1 * \tau_2) * \tau_3 &: \assocrt \\ [1.5ex]
...
language with quantum control~\cite{10.1007/978-3-319-89366-2_19}.
Theseus~\cite{james2014theseus} is a language that layers
convenient syntactic extensions over an even more basic language $\Pi$ of
combinators witnessing isomorphisms over finite
types~\cite{DBLP:conf/esop/CaretteS16}. The
...
categories}~\cite{kelly1972many}. These categories extend the monoidal
categories~\cite{XXX} which are used to model many resource-aware
programming languages~\cite{XXX}
\ednote{The search on
"monoidal category" "resource-aware" "programming language" on Google
Scholar produces just 6 papers!}
...
conventional data. Universal computation models however
...
simpler setting of the core $\Pi$ language. First, simple facts
about cardinalities:
...
::  𝕋 :: [] [ Fin.0F ] or 𝔽 :: 𝕋 :: [] [ Fin.1F ] depending on whether the
values is \AgdaInductiveConstructor{false} or \AgdaInductiveConstructor{true}.
The prepended list
...
\AgdaInductiveConstructor{false} when applies to a non-zero natural
...
show. What is more interesting is \AgdaFunction{test3} which shows a
...
\ednote{JC: Doesn't \AgdaFunction{eval}, in the type, evaluate
fully for this work?  That Agda is strongly normalizing is rather
important here. What exactly is partially evaluating is really not
clear (says an expert in PE). I think the idea being expressed is
correct, but the paragraph is insufficiently clear/precise.}
