...
\noindent The circuit exploits the additional expressiveness of
choosing the initial value of the ancilla (and sets it to true). When
applied to quantum inputs, the circuit detects whether the two qubits
are entangled in the state
$\frac{1}{\sqrt{2}}(\ket{00}+\ket{11})$. This property can be
confirmed from the classical behavior: if the inputs are anything
else, the ancilla bit is not restored to its original value (true) and
the dynamic check fails.
