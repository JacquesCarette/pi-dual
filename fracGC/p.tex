...
ancillae. The circuit has one input and one output. Immediately after
receiving the input, the circuit generates an ancilla wire and its
corresponding gc process (first line in the Agda definition). The
original input and the ancilla wire interact using two
\AgdaFunction{CNOT} gates, after which the ancilla wire is redirected
to the output (second line in the Agda code). Finally the original
input is gc'ed (third line in the Agda code). The entire circuit is
extensionally equivalent to the identity function.
