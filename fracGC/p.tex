...
Figure~\ref{fig:trace-id} shows a trace of the execution of \AgdaFunction{id'}
from Sec.~\ref{sub:id} applied to 𝕋:

\begin{figure}[htb]
\includegraphics[scale=0.4]{trace2.png}
\caption{Execution trace of \AgdaFunction{id'}}
\label{fig:trace-id}
\end{figure}
...
just write programs in that style: indeed the dual nature of proofs
...
all the information needed for the proof invariants. In our case, the
question is whether we can extract a program that runs in the XX but
is guaranteed to be safe and hence can run without the runtime checks
associated with de-allocation sites.
