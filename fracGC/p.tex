...
proving universality (i) assume that an unlimited amount of temporary
storage may be
used~\cite{Toffoli:1980,DBLP:conf/innovations/AaronsonGS17}, and (ii)
that this temporary storage is returned to its initial state before
de-allocation. The latter condition is necessary in order maintain
symmetry between input and output in reversible circuits and to avoid
decoherence problems in quantum computing, Unfortunately, in current
proposals for reversible and quantum programming languages
(e.g. Quipper~\cite{Green:2013:QSQ:2491956.2462177}), this condition
is at the programmer's discretion.
