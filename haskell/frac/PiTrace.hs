...
class Eq a => Fin a where
  elems :: [a]

instance Eq Zero where
  _ == _ = undefined
  
instance Fin Zero where
  elems = []
...
-- Congruence
  Id    :: Fin a => a :<=> a
  Sym   :: (Fin a, Fin b) => (a :<=> b) -> (b :<=> a) 
  (:.:) :: (Fin a, Fin b, Fin c) => (a :<=> b) -> (b :<=> c) -> (a :<=> c)
  (:*:) :: (Fin a, Fin b, Fin c, Fin d) => (a :<=> b) -> (c :<=> d) -> ((a,c) :<=> (b,d))
  (:+:) :: (Fin a, Fin b, Fin c, Fin d) => (a :<=> b) -> (c :<=> d) -> (Either a c :<=> Either b d)
-- (+) is associative, commutative, and has a unit
  ZeroE   :: Fin a => Either Zero a :<=> a
  ZeroI   :: Fin a => a :<=> Either Zero a
  SwapP :: (Fin a, Fin b) => Either a b :<=> Either b a
  AssocLP  :: (Fin a, Fin b, Fin c) => Either a (Either b c) :<=> Either (Either a b) c 
  AssocRP  :: (Fin a, Fin b, Fin c) => Either (Either a b) c :<=> Either a (Either b c) 
-- (*) is associative, commutative, and has a unit
  UnitE    :: Fin a => ((), a) :<=> a
  UnitI    :: Fin a => a :<=> ((), a)
  SwapT :: (Fin a, Fin b) => (a,b) :<=> (b,a) 
  AssocLT  :: (Fin a, Fin b, Fin c) => (a,(b,c)) :<=> ((a,b),c)
  AssocRT  :: (Fin a, Fin b, Fin c) => ((a,b),c) :<=> (a,(b,c))
-- (*) distributes over (+) 
  DistribZ  :: Fin a => (Zero, a) :<=> Zero
  FactorZ  :: Fin a => Zero :<=> (Zero, a)
  Distrib  :: (Fin a, Fin b, Fin c) => (Either b c, a) :<=> Either (b, a) (c, a)
  Factor      :: (Fin a, Fin b, Fin c) => Either (b, a) (c, a) :<=> (Either b c, a)
-- Encoding of booleans
  FoldB   :: Either () () :<=> Bool
  UnfoldB :: Bool :<=> Either () ()
-- Trace operators for looping/recursion
  TraceP :: (Fin a, Fin b1, Fin b2) => (Either a b1 :<=> Either a b2) -> (b1 :<=> b2)
  TraceT :: (Fin a, Fin b1, Fin b2) => ((a, b1) :<=> (a, b2)) -> (b1 :<=> b2)
...
evalL :: (Fin a, Fin b) => (a :<=> b) -> [a] -> [b]
evalL c as = nub $ as >>= (eval c)

evalRL :: (Fin a, Fin b) => (a :<=> b) -> [b] -> [a]
...
cond :: (Fin a, Fin b) => (a :<=> b) -> (a :<=> b) -> ((Bool, a) :<=> (Bool, b))
cond f g = -- T,F
  (UnfoldB :*: Id) :.: 
  Distrib :.: 
  ((Id :*: f) :+: (Id :*: g)) :.: 
...
------------------------------------------------------------------------
-- Multiplicative Trace (LP)
        
{--        
relation r {(F,F),(F,T),(T,F)} can be realized as follows
idea is to find a combinator c as follows and then trace

c (FF,F) = (FF,F)
c (FF,T) = (TF,T) (FF,T)
c (FT,F) = (FT,T) (TT,T) (FT,F)
c (FT,T) = (FT,F) (FT,F) (FT,T)
c (TF,F) = (TT,F) (TT,F) (TT,T) (TF,F)
c (TF,T) = (TT,T) (FT,T) (TT,F) (TF,T)
c (TT,F) = (TF,F) (TF,F) (TF,F) (TT,F)
c (TT,T) = (FF,T) (TF,T) (TF,T) (TT,T)

--}


cr :: ((Bool,Bool),Bool) :<=> ((Bool,Bool),Bool)
...
annihilate :: Fin a => a :<=> a
annihilate = TraceT c
  where c :: Fin a => (Bool, a) :<=> (Bool, a)
        c = inot :*: Id
...
twice c = TraceP (((c :+: c) :+: Id) :.: r)
  where r :: Fin a => (Either (Either a a) a) :<=> (Either (Either a a) a)
        r =                  -- (a + b) + c
          (SwapP :+: Id) :.: -- (b + a) + c
          AssocRP :.:        -- b + (a + c)
          (Id :+: SwapP) :.: -- b + (c + a)
