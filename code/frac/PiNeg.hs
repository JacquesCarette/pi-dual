...
data Zero 
data Neg a = Neg a deriving (Show, Eq)
...
-- Eta/Psi
  EtaP :: (B a) => () :<=> Either (Neg a) a
  EpsP :: (B a) => Either (Neg a) a :<=> ()
...
eval EtaP _ = [  Left (Neg v) | v <- elems ] ++ [ Right v | v <- elems ]
eval EpsP (Left (Neg v)) = undefined
eval EpsP (Right v) = undefined
...
evalR EtaP (Left (Neg v)) = undefined
evalR EtaP (Right v) = undefined
