{-# LANGUAGE TypeOperators, GADTs, TypeSynonymInstances, NamedFieldPuns #-}
...
-----------------------------------------------------------------------
-- Isomorphisms between VALUES

data Recip a = Recip a deriving (Show, Eq)

-- the individual clauses relating values to values
...
data v1 :<-> v2 where
  ...
  SwapBool1 :: Bool :<-> Bool
  SwapBool2 :: Bool :<-> Bool
  EtaBool1  :: () :<-> (Recip Bool,Bool)
  EtaBool2  :: () :<-> (Recip Bool,Bool)
  EpsilonBool1 :: (Recip Bool,Bool) :<-> ()
  EpsilonBool2 :: (Recip Bool,Bool) :<-> ()

eval :: (v1 :<-> v2) -> v1 -> v2
eval IdBool1 False = False
eval IdBool2 True = True
eval SwapBool1 False = True
eval SwapBool2 True = False
eval EtaBool1 () = (Recip False,False)
eval EtaBool2 () = (Recip True,True)
eval EpsilonBool1 (Recip False,False) = ()
eval EpsilonBool2 (Recip True,True) = ()

-- a type is a set values {v1, v2, ...} TOGETHER with the equalities 
-- v1 :<-> v2 which witness how various values are related to each other

data Typ a = Typ { elems :: [a], eq :: [a :<-> a] }
...
boolTyp :: Typ Bool 
boolTyp = Typ { elems = [False,True],
                eq = [IdBool1,IdBool2,SwapBool1,SwapBool2] }

-- Now we have isomorphisms between TYPES; this is the level of isomorphisms
-- we've always had

data b1 :<=> b2 where
  Swap :: Typ Bool :<=> Typ Bool

evalB :: (b1 :<=> b2) -> b1 -> b2
evalB Swap (Typ{elems,eq}) = undefined -- ??
