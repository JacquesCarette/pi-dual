...
In a recent paper~\cite{infeffects}, we argued that, by encompassing
irreversible physical primitives, conventional abstract models of computation
have also inadvertently included some \emph{implicit} computational effects
which we called \emph{information effects}. We then developed a pure
reversible model of computation that is obtained from the type isomorphisms
and categorical structures that underlie models of linear logic and quantum
...
\end{array}\]
Although seemingly pointless, this isomorphism has a non-trivial and
interesting computational interpretation (see Sec.~\ref{sub:neg}). 

corresponds to a type isomorphism between the object $a$ and itself. 
...
Specifically, 
isomorphisms enrich our computational model with have an interesting
...
On the left we have a producer
that can supply a value of type $a$ and on the right we have a consumer that
demands a value of type $a$. For the purposes of the example, think of the
producer as a buyer who has \$20.00 and the consumer as a seller who expects
\$20.00 for the merchandise. In a conventional computational model, one might
realize this situation by simply writing the identity function: the buyer
hands the money to the seller to finish the transaction. The above sequence
of isomorphisms implements this identity function in a much more interesting
way, however. 
of the above series of isomorphisms is that money is ``created'' on the
consumer side together with a corresponding debt that travels ``backwards''
until it is paid for by the money at the supplier. In other words, this
computation models a credit card transaction. It shares the same benefits of
such transactions: allowing transactions to occur without necessarily
demanding that the resources be present at the time of the transaction.

On the producer side, the debt is paid for by the money computation with
the identity function. the producer and consumer must somehow share an
explicit dependency that allows the value. However in our model, the presence
of negative types allows the produced value to satisfy the demand without the
producer or consumer even knowing about each other. As is explained in detail
in

Indeed, in 
the above example, the computation on the consumer side creates 
...
Combinators of {{langRev}} can be written using the the operators
described in definition \ref{def:langRev} or via a graphical language
similar in spirit to those developed for GoI
\cite{DBLP:conf/popl/Mackie95} and for category theory
\cite{springerlink:10.1007/978-3-642-12821-94}. Modulo some
conventions and shorthands we will describe here, the wiring diagrams
are a complete and equivalent to the operator based (syntactic)
description of programs.  {{langRev}} combinators expressed in this
graphical language looks like a ``wiring diagrams''. Value take the
form of ``particles'' that flow along the wires. Computation is
...
three parallel wires maybe inferred as {{b1*(b2*b3)}} or
{{(b1*b2)*b3}}, based on the context. This is much like handling of
...
\item
Identity morphisms and their duals are represented as follows:
...
of type {{b^^^}}. These two values are free to move independantly
...
This also leads to the exiciting point of view that entangled
particle/anti-particle pairs act as functions in a physical
first-class sense. We can ask the fuction to be applied to a value by
...
fixpoint of the contraint (expressed with combinators) that is
...
world, we have to supply additional information in teh form of
...
generalises to any constraint satisfaction problem whose search space
