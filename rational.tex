...
We start with the semantics for {{langRev}} defined in Sec
\ref{sec:pi}, which we will systematically rewrite with unification
and then present a ``reverse'' evaluator written in the same style. We
shall then combine these into the semantics for {{langRevEE}}.

TODO. define substitutions and unification. 
...
%! columnStyle = rclr
%! fwd = \triangleright
% <iso, v, cc, s>_{fwd} &|-->& {[iso, v', cc, s']}_{fwd}
% & & where iso s v |--> (v', s')
% <c1(;)c2, v, cc, s>_{fwd} &|-->& <c1, v, Fst cc c2, s>_{fwd}
% {[c1, v, Fst cc c2, s]}_{fwd} &|-->& <c2, v, Snd c1 cc, s>_{fwd}
% {[c2, v, Snd c1 cc, s]}_{fwd} &|-->& {[ c1(;)c2, v, cc,s ]}_{fwd}
% <c1(+)c2, v', cc, s>_{fwd} &|-->& <c1, v, LeftPlus cc c2, s[v' <><> left v]>_{fwd}
% {[ c1, v, LeftPlus cc c2, s ]}_{fwd} &|-->& {[c1 (+) c2, left v, cc,s ]}_{fwd}
% <c1(+)c2, v', cc, s>_{fwd} &|-->& <c2, v, RightPlus c1 cc, s[v' <><> right v]>_{fwd}
% {[ c2, v, RightPlus c1 cc,s ]}_{fwd} &|-->& {[c1 (+) c2, right v, cc,s ]}_{fwd}
% <c1(*)c2, v', cc, s>_{fwd} &|-->& <c1, v1, LeftTimes cc c2 v2, s'>_{fwd}
% & & where s' = s[v <><> (v1, v2)]
% {[ c1, v1, LeftTimes cc c2 v2,s ]}_{fwd} &|-->& <c2, v2, RightTimes c1 v1 cc, s>_{fwd}
% {[ c2, v2, RightTimes c1 v1 cc,s ]}_{fwd} &|-->& {[ c1 (*) c2, (v1, v2), cc,s ]}_{fwd}
...
%! columnStyle = rclr
%! bck = \triangleleft
% {[iso, v, cc, s]}_{bck} &|-->& <iso, v', cc, s'>_{bck}
% & & where iso^{dagger} s v |--> (v', s')
% <c1, v, Fst cc c2, s>_{bck} &|-->& <c1(;)c2, v, cc, s>_{bck}
% <c2, v, Snd c1 cc, s>_{bck} &|-->& {[c1, v, Fst cc c2, s]}_{bck}
% {[ c1(;)c2, v, cc, s ]}_{bck} &|-->& {[c2, v, Snd c1 cc, s]}_{bck}
% <c1, v, LeftPlus cc c2,s >_{bck} &|-->& <c1(+)c2,left v, cc>_{bck}
% {[c1 (+) c2, v', cc, s ]}_{bck} &|-->& {[ c1, v, LeftPlus cc c2, s[v' <><> left v] ]}_{bck}
% <c2, v, RightPlus c1 cc, s>_{bck} &|-->& <c1(+)c2, right v, cc>_{bck}
% {[c1 (+) c2, v', cc,s ]}_{bck} &|-->& {[ c2, v, RightPlus c1 cc, s[v' <><> right v] ]}_{bck}
% <c1, v1, LeftTimes cc c2 v2, s>_{bck} &|-->& <c1(*)c2, (v1, v2), cc, s>_{bck}
% <c2, v2, RightTimes c1 v1 cc, s>_{bck} &|-->& {[ c1, v1, LeftTimes cc c2 v2, s ]}_{bck}
% {[ c1 (*) c2, v, cc, s ]}_{bck} &|-->& {[ c2, v2, RightTimes c1 v1 cc, S' ]}_{bck}
% && where s' = s[v <><> (v1, v2)]
...
%! fwd = \triangleright
%! bck = \triangleleft
% <eps+, v, cc, s>_{fwd} |--> <eps+, left (-v'), cc, s[v <><> right v]>_{bck}
% <eps+, v, cc, s>_{fwd} |--> <eps+, right v', cc, s[v <><> left (-v')]>_{bck}
...
%! fwd = \triangleright
%! bck = \triangleleft
% <eta+, v, cc, s>_{bck} |--> <eta+, left (-v'), cc, s[v <><> right v]>_{fwd}
% <eta+, v, cc, s>_{bck} |--> <eta+, right v', cc, s[v <><> left (-v')]>_{fwd}
