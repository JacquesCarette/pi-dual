...
In a compact closed category, each morphism {{f : a <-> b }} can be given a
\emph{name} and a \emph{coname}. For the additive fragment, the name
$\fname{f}$ has type {{0 <-> (-a + b)}} and the coname $\fconame{f}$ has type
{{a + (-b) <-> 0}}. For the multiplicative fragment, the name $\fname{f}$ has
type {{1 <-> ((1/a) * b))}} and the coname $\fconame{f}$ has type 
{{(a * (1/b)) <-> 1}}. Intuitively, this means that for each morphism, 
...
\paragraph*{Lifting an operation of positive types to negated types.}
Any operation on types can be applied to the negative versions of these
types. The idea is simply to reverse the flow of values and use the
...
This geometry was developed by Girard~\cite{girard1989geometry} as part of
