...
value {{20}}, at line (2) with the value {{right (right 20)}} flowing forward
and the value {{right (left 20)}} flowing backwards, and at line (3) with the
value {{left (left 20)}} flowing forward and the value {{left (right 20)}}
flowing backward. We now have three independent subcomputations proceeding in
...
\item At this point, we have produced the skeleton for our desired function.
  ...
  \begin{itemize}
    ...
    \item the first component of the pair then reduces to () and can be
      ...
      available;
    \item the first clause of the function has now disappeared and the the
      unification adjusts the remaining two clauses to
    {{((1/alpha2,alpha3),(1/alpha3,R))}}.
  \item The same client or another client can now use the second clause
    applying it to {{G}} which would produce the result {{alpha3}} for the
    second client and ground the result of the first client to {{G}}.
...
like the case for negative types, a dual reading of the the pattern-matching
...
which computations can be sliced and diced, decomposed and recomposed,
proceeding forward and backwards, in arbitrary ways. The remainder of the
paper formalizes these informal observations. Specifically our main
contributions are:
\begin{itemize}
...
% {{a <-> a}} isomorphism computes as follows:
% \begin{itemize}
...
\roshan{Say somewhere that {{langRev}} corresponds to computing in the
  commutative rig of whole numbers. More bluntly {{langRev}} is whole
  numbers with addition and multiplication, while {{langRevEE}} is the
  rational with addition and multiplication.}
...
lacks a multiplicative tensor that distributes over the additive
tensor in Int {{langRevT}}.
...
\item Negative and fractional types have an elementary and familiar
  interpretation borrowed from the algebra of rational numbers. One
  can write any algebraic identity that is valid for the rational
  numbers and interpret it as an isomorphism with a clear
  computational interpretation: negative values flow backwards and
  fractional values represent fragments of aggregate data
  structures. None of the systems above has such a natural
