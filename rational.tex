...
possible to directly connect the producer and consumer: such a connection
would be modeled by the identity function. The proof above implements this
connection in a much more interesting way, however. A transformation $0 =
(-a) + a$ is used at the consumer site to create money and a corresponding
...
\item As the example illustrates, negative types corresponds to ``debts.''
  ...
  complicated mechanisms are typically needed to constrain the propagation
  and use of negative values because the surrounding computational framework
...
\item The appeal of negative types can already be gleaned from the credit
  ...
  because they disentangle the propagation of the money from the propagation
  ...
  the money will be paid at some point, as long as the infrastructure can be
  trusted that such promises will be fulfilled. 
\end{itemize}

Similar observations hold for fractional types but we defer these to
Sec.~\ref{sub:frac}. Before proceeding to the main part of the paper, we
briefly summarize the our technical contributions:
\begin{itemize}
\item 
\item 
\item 
