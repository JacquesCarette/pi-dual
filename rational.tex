...
The {{inv}} construction is similar.  We are able to do this because
in {{langRevEE}} every morphism is an isomorphism and hence has an
...
\item To add negative types we add the following rules to the
  ...
  \item The rules for {{eps+}} essentially transfer control from the
    ...
    to be negative and transfered to the {{left}} wire, and vice versa.
%subcode{opsem} include main
%! columnStyle = rclr
%! fwd = \triangleright
%! bck = \triangleleft
...
% <eps+, v, C, s>_{fwd} |--> <eps+, right v', C, s[v <><> left (-v')]>_{bck}
   %
    ...
    \item The rules for {{eta+}} are added to backward evaluator. A
      program executing backward starts executing forward after the
