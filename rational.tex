...
variables that capture constraints and hence it useful to trace this
computation with the analogy to logic variables in mind:
...
\verb|color| {{R}} in one subcomputation, fixes it in the
other. Depending on the choice of the type {{a}}, an arbitrary number
of bits may be communicated. The dual reading of the above, with
additive functions of the form {{0<-> (-a)+b}}, correspond to
control flow and coroutining between two subcomputations -- only one
of which `exists' at any point.
...
%% That said, we will focus on the limited case of boolean satisfiability
%% here.
...
%% Partial isomorphisms algebraics
%% do have computational 

%% While this equation does indeed have an irrational imaginary root,
%% as we will see -- {{langRevEE}} does not have the means of
%% expressing meaningful computation that uses the root.
...
%% Jacques Carette, Michael Adams, Khaled
