...
\item The appeal of negative types can already be gleaned from the credit
  ...
  that the money will be paid at some point, as long as the infrastructure
  can be trusted that such promises will be fulfilled. Computationally, this
  means that negative types allow us to break the dependency between the
  producer and consumer and let each compute independently.
\end{itemize}
...
\item We extend {{langRev}} our reversible programming language of type
  isomorphisms~\cite{rc2011,infeffects} with a notion of negative types, that
  satisfies the isomorphism $a + (-a) = 0$. The semantics of this extension
  is expressed by having a ``dual'' evaluator that reverses the flow of
  execution for negative values. 
...
\item We develop programming intuition and argue that negative and fractional
  types ought to be part of the vocabulary of every programmer.
\item We relate our notions of negative and fractional types to previous work
  on continuations. We analyze and relate two apparently unrelated lines of
  work: the first pioneered by
  Filinski~\cite{Filinski:1989:DCI:648332.755574} relates continuations to
  negative types; the second~\cite{Bernardi:2010:CSL:1749618.1749689} relates
  continuations to the fractional types of the Lambek-Grishin calculus.
\end{itemize}
...
determined. It is clear that, at an intuitive level, acceptors are entities
that combine elements of our negative and fractional types.
...
Wadler the reloaded paper, does not consider the subtraction type because its
``computational interpretation is not familiar.'' Curien and Herbelin study
duality in classical logic, show that it exchanges call-by-value and
call-by-name. They extend classical natural deduction with subtraction but
give it no computational meaning. Crolard (in the formulae-as-types
interpretation of subtractive logic) address the computational interpretation
of subtraction. He explains the type $A-B$ as the of \emph{coroutines} with a
local environment of type $A$ and a continuation of type $B$. The description
is complicated by what is essentially the desire to enforce linearity
constraints so that coroutines cannot access the local environment of other
coroutines. Must cite Selinger control categories in this context of duality
but I am not sure what to say: it assumes cartesian closed categories for one
thing and not symmetric monoidal ones. Ariola, Herbelin, and Sabry also use
subtractive types to explain delimited continuations.

\paragraph*{Polarized Logic.} Polarized logic is the logic of values and
continuations: each connective can be given a positive or negative polarity,
which can be related to evaluation
order~\cite{10.1109/LICS.2010.23}. Negative types in this work are a mix of
our negative and fractional types: they include a notion of demand which
corresponds to our negative types and a notion of pattern-matching on
continuations (i.e., a notion of constraints on the context) which
corresponds to our fractional types.
...
types: product, left division, right division, coproduct, right difference,
and left difference~\cite{Bernardi:2010:CSL:1749618.1749689}. The division
and difference types are similar to our types but because the calculus lacks
...
contract, our two symmetries relate sums to subtractions and products to
divisions.

\newpage

Curien and Herbelin~\cite{Curien:2000} introduce a calculus that exhibits
...
$B$. which whose computational interpretation is left for future work. Curien
and Herbelin informally explain values of types $A-B$ either as a context
expecting a function $A \rightarrow B$ or as a \emph{pair} containing a value
and a continuation. In our interpretation, a value of type $A-B$ is
\emph{either} a value of $A$ or a demand for a value of type $B$.  
