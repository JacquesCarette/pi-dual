...
\item Each of the values {{-a + b}} and {{(1/a) * b}} can be viewed as a
  ...
  {{b}} and then demanding an {{a}} and in that perspective they correspond
  to delimited continuations. Evidently, as the discussion above suggests,
  these two notions of functions are not the same at all and should not be
  conflated. Sec.~\ref{sub:hof} discusses this point in detail.
...
For a monoidal category to be compact closed, the maps {{eta}} and {{eps}}
must satisfy a coherence condition that is usually visualized as follows
\begin{center}
  \includegraphics{diagrams/coherence.pdf}
\end{center}
In the case of negatives, the condition amounts to checking that reversing
direction twice is a no-op. In the case of fractionals, the condition amounts
to checking that creating values {{alpha}} and {{1/alpha}} and immediately
unifying them is also a no-op. Both checks are straightforward.
...
the following definition.  Given {{f : a+c <-> b+c }}, 
define {{trace+ f : a <-> b}} as:

{{ trace+ f = zeroi (;) (id (+) eta+) (;) (f (+) id) (;) (id (+) eps+) (;) zeroe }} 

We have omitted some the commutativity and associativity shuffling to
communicate the main idea. We are given a value of type {{a}} which we embed
into {{a+0}} and then {{a + (c - c)}}. This can be reassociated into 
{{(a+c) - c}}. The component {{a+c}} which until now is just an appropriately 
tagged value of type {{a}} can be given to {{f}} which transforms it to
something of type {{b+c}}. If the result is in the {{b}}-summand, it is produced
as the answer; otherwise the result is in the {{c}}-summand; {{eps+}} is used to
make it flow backwards to be fed to the {{eta+}} located at the beginning of the 
sequence. Iteration continues until a {{b}} is produced.
...
We write these type
written {{a-o*a}} whose witness may be thought of as the identity function at
the type {{a}}.  For example, consider a datatype {{color = R|Gr|B}}, and let
...
using the adjoint of {{f}}, denoted {{f^{dagger}}}. At this point we have,
