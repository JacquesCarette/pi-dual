...
interpretation. Consider the starting $a$ as being supplied by a producer of
values and the final $a$ as being demanded by a consumer. For the purposes of
the example, think of the producer as someone having \$20.00 and the consumer
as someone demanding \$20.00. It is certainly possible to directly connect
the producer and consumer using the algebraic identity $a=a$: such a
connection would be computationally modeled by the identity function as
expected. However, the algebraic proof above implements this connection in a
much more interesting way. As Sec.~\ref{sec:neg} explains in detail, the
flow of the computation is as follows:
\begin{itemize}
...
Collecting the various events that happen at the same lines, we can
reorganize the above computation as follows:
\begin{itemize}
\item At line 0, we have the value {{20}} flowing forward;
\item At line 1, we have the value {{left 20}} flowing forward;
\item At line 2, we have the value {{left 20}} flowing forward or the value
  {{right (left 20)}} flowing backwards or the value {{right (right 20)}}
  flowing forward;
\item At line 3, we have the value {{left (left 20)}} flowing forward or the
  value {{left (right 20)}} flowing backward or the value {{right 20}}
  flowing forward;
\item At line 4, we have the value {{right 20}} flowing forward;
\item At line 5, we have the value {{20}} flowing forward.
\end{itemize}

Use the above to argue that we have something like a credit card transaction!

the transformation $0 = (-a) + a$ is used at the
consumer site to create money and a corresponding debt. The consumer gets the
money immediately and the debt travels ``backwards'' towards the producer
where it is paid.  In other words, this computation models something similar
to a credit card transaction. We discuss this example in detail in
Sec.~\ref{sub:neg} and only focus on two important points here:
\begin{itemize}
