...
{{langRevTF}} in the category of finite sets and relations. This category is
an example of a \emph{biproduct dagger compact closed category} which are
...
computation.\footnote{The other common example of these categories is that of
  finite dimensional Hilbert spaces.} The category intuitively supports
higher-order relations as any relation between $A$ and $B$ can be represented
as a subset of {{A * B}}, i.e., as a value. More generally, the two main
features of these biproduct dagger compact closed categories are that they
support an adjoint for each morphism (to model reversibility) and a notion of
higher-order relation (built from the dagger which is modeled by our
fractional types).

To interpret {{langRevTF}} in this category, we associate a finite set
$\llbracket b \rrbracket$ with each type $b$ and a relation $\llbracket c
\rrbracket$ with each combinator $c$. The denotation of types is
straighforward: the denotation of {{1/b}} is an isomorphic copy of the
denotation of {{b}}. 
...
Perhaps get rid of the semantics below and replace by a semantics which maps
each combinator to a relation. This is probably clearer than the operational
style.
...
a form of non-determinism in the sense that there could be zero, one, or several
