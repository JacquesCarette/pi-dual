...
context for our higher-order extension.\footnote{The presentation in this
  ...
  include the empty type, recursive types, and trace opeators but these
...
regarded as largely ancilliary.  In particular we do not treat the above
...
semantics because we present a more general version in the next section. We
state without proof that the evaluation of well-typed combinators always
terminates and that {{langRev}} is logically reversible, i.e., that for all
combinators {{c : b1 <-> b2}} and values {{v1 : b1}} and {{v2 : b2}} we have
the forward evaluation of {{c v1}} produces {{v2}} iff the backwards
...
%subcode{bnf} include main
% natural numbers, n 
% value types, b ::= 1 | b+b | b*b | 1/b | bool 
