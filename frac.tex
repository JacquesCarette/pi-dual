...
%    output.
%
% Roshan: I completely understand this and this is not my objection. This
% is not what the above sentence is saying however. Its talking about
% Pi not having a notion of input and output which is simply
% confusing.
...
% Roshan: it reads much better now. 
...
\roshan{Either we should have explain the SAT solver well or we
  shouldn't have at all. There are some original ideas in the
  construction of that solver and the Oxford QF group for instance
  were very interested in its details. This presentation neither
  leaves the idea prime for future publication, and neither does it
  explain it well.}
...
\roshan{Again, if we have lost zero, we cannot talk about vector
  spaces with the zero vector. We should alternately included it in
  our presentation to start with. }
...
\roshan{The above para should simply be deleted.}
...
\roshan{Again, the above para is just lose speculation. Further, the
  fact that we say that its cartesian closed instead of monoidal
  closed should *immediately* tell us that its the wrong model.}
