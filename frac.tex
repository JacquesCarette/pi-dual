...
{{langRevTF}} in \emph{biproduct dagger compact closed categories}. These
categories were introduced by Abramsky and
Coecke~\cite{Abramsky:2004:CSQ:1018438.1021878} and then by
Selinger~\cite{Selinger:2007:DCC:1229185.1229207}. The bare definitions are
collected in Appendix~\ref{app:cat}.

The simplest example of such categories is the category of finite sets and
relations on which we focus. To interpret {{langRevTF}} in this category, we
associate a set with each type. For {{1}}, we associate a set with one
element and we interpret {{*}} as the cartesian product and {{+}} as the
disjoint union. For {{1/b}}, we associate the same set associated with {{b}}
and hence, in our denotations, we identify {{b}} and {{1/b}} as well as {{v}}
and {{1/v}}. It is intuitively clear that this category is compact closed as
any relation between $A$ and $B$ can be represented as a finite set of type
{{A * B}}. For example, consider the ``boolean negation'' relation:

{{ {[ (1/left(), right()), (1/right (), left ()) ]} }}

This is clearly isomorphic to the corresponding set of pairs. The map
{{eta_b}} constructs the identity relation at type {{b}}. For example, {{eta_{1+1} }}
is the relation:
