...
\noindent
where {{reorder}} is the obvious combinator of type {{b1 * (b2 * b3) * b4 <-> b1
* (b3 * b2) * b4}}. At the type level, the computation proceeds as follows:

%% XXX: why won't the columnstyle declaration here work? (Z)
%subcode{opsem} include main
%! columnStyle = rlr
%        & 1/(b1 * b2) & 
%     <->& 1 * 1/(b1 * b2) & (uniti)
%     <->& 1 * (1 * 1/(b1 * b2)) & (uniti)
%     <->& (1 * 1) * 1/(b1 * b2) & (assocl*)
%     <->& ((1/b1 * b1) * (1/b2 * b2)) * 1/(b1 * b2) &((eta*_{b1} (*) eta*_{b2}) (*) id)
%     <->& ((1/b1 * 1/b2) * (b1 * b2)) * 1/(b1 * b2) & (reoder (*) id)
%     <->& (1/b1 * 1/b2) * ((b1 * b2) * 1/(b1 * b2)) & (assocr*)
%     <->& (1/b1 * 1/b2) * (1/(b1 * b2) * (b1 * b2)) & (id * swap*)
%     <->& (1/b1 * 1/b2) * 1 & (id * eps*)
%     <->& 1 * (1/b1 * 1/b2) & (swap*)
%     <->& (1/b1 * 1/b2) & (unite)
