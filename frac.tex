...
computation.\footnote{The other common example of these categories is that of
  finite dimensional Hilbert spaces.} The two main features of these
biproduct dagger compact closed categories are that they support an adjoint
for each morphism (to model reversibility) and that they support higher-order
relation (built from the categorical dagger which is modeled by our
fractional types). The latter is intuitively clear as any relation between
$A$ and $B$ can be represented as a subset of {{A * B}}, i.e., as a
value. 
...
The denotation of types is straighforward: the denotation of {{1/b}} 
is an isomorphic copy of the denotation of {{b}}.
