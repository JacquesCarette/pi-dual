...
The extension of the sets of types and values is simple:
...
where {{v : b}}. Note that {{1/v}} is purely formal, as is {{1/b}}. There is
however have some informal intuition about the \emph{meaning} of such
fractional types and values. (See Sec.~\ref{sec:cat} for more discussion
about this point.) From both the programming perspective and from the
categorical perspective we expect the fractional types to satisfy the
following isomorphism witnessed by two new combinators {{eta*_b}} and
{{eps*_b}}:

{{eta*_b : 1 <-> 1/b * b : eps*_b}}

From a programming perspective, we think of a value {{1/v}} as a
...
some information of a specific shapre (i.e., \emph{negative information}),
...
information. From the categorical perspective, the extended language {{PiTF}}
needs to have a semantics in a \emph{compact closed category} which are the
categories introduced by Abramsky and
Coecke~\cite{Abramsky:2004:CSQ:1018438.1021878} and by
Selinger~\cite{Selinger:2007:DCC:1229185.1229207} to model quantum
computation. Two of the most relevant features of these categories for our
purposes are that they support an adjoint for each morphism (to model
reversibility) and that they support higher-order relation (built from the
categorical dagger which is modeled by our fractional types). The main
requirement on the dagger operation is the isomorphism above witnessed by
{{eta*_b}} and {{eps*_b}}. 

One can think of several concrete compact closed categories: the first and
most intuitive is the category of finite sets and relations. In that case,
the dagger operation (i.e., the fractional type) collapses by defining
{{1/b}} to be {{b}}. This is still an interesting category as any relation is
trivially reversible and more importantly any relation between {{b1}} and
{{b2}} can be represented as a subset of {{b1 * b2}}, i.e., as a value. A
more interesting example is the category of finite dimensional vector spaces
and linear maps (or even the category of finite dimensional Hilbert
spaces). In such categories the dagger operation takes a vector space
(consisting of ``column vectors'') to the dual vector space (consisting of
``row vectors''). These vector spaces are isomorphic, but they are
nevertheless quite different, being $(1,0)$ and $(0,1)$ tensors
(respectively).  In other words, the category provides a more refined model
in which isomorphic negative-information values and positive-information are
not identified. 
...
{{c :  b1 <-> b2}} as a \emph{relation} between the sets denoted
by {{b1}} and {{b2}} respectively. As we illustrate, this semantics allows
\emph{arbitrary} relations (including the empty relation and
relations that are not isomorphisms) to be represented as values in {{PiTF}}.
In Sec.~\ref{sec:cat} we discuss a more refined semantics that is
more suitable for richer categories and which only allows relations
representing \emph{isomorphisms} to be represented as values in~{{PiTF}}. 
...
backwards evaluation of {{c v2}} produces {{v1}} as a possible answer. 
