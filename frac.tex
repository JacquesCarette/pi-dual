...
\begin{abstract}
  ...
  information-preserving, and reversible programming language {{Pi}}
  ...
  paper, we introduce a higher-order extension of {{Pi}} based on the
...
languages, {{Pi}} with various superscripts, in which every computation
...
also invested a significant amount of time in using {{Pi}} as an actual
programming language and established several idioms for programming large
classes of interesting (recursive) programs in {{Pi}}.
...
Mathematically, {{Pi}} can be interpreted in the category of finite sets
...
denoted by {{b1}} and {{b2}}. Operationally, the semantics of {{Pi}} is
...
that {{Pi}} is logically reversible, i.e., that for all combinators 
...
Note that a small modification of {{Pi}} (the addition of an
...
but would correspond to certain identity types being trivial.  {{Pi}}
...
In {{Pi}} presented in Sec.~\ref{sec:pi}, we have been able to maintain
...
\begin{verbatim}
In the rules above it is unclear as to which v is chosen in the rule
 for eta. It is unclear that these rules are logically reversible.
\end{verbatim}
...
% swap* '= {[ ((v1, v2), (v2, v1)) ~|~ v1 in b1, v2 in b2 ]}
% swap+ '= {[ ((left v1), (right v1)) and ((right v2), (left v2)) ~|~ v1 in b1, v2 in b2]}
% assocl* '= {[ ((v1, (v2, v3)), ((v1, v2), v3)) ~|~ v1 in b1, v2 in b2, v3 in b3]}
etc..
...
We can define evaluation as follows: 
%
{{ c v = {[ v' ~|~ (v, v') in c ]} }}. 

However this is unpleasant since it is unclear how to evaluate if
{{c~v}} and {{c^{dagger} v'}} are dual by evaluation. This ties into a
deeper issue of interpreting relations as programs.

Q) If {{|b1|=|b2|}}, can we define all relations between {{b1}} and
