...
%%%%%%%%%%%%
\subsection{Trace Operators and (Finite) Relational Programming}
...
Consider the combinator {{id : bool * 1 <-> bool * 1}}.
Using {{trace*}}, we construct the combinator {{trace* c : 1 <-> 1}}.
Applying this to {{() : 1}} requires us to find a value {{b : bool}} such
that {{(b,()) ~~d~~ (b,())}}. Given that the type {{bool}} has two values,
there are two values {{false}} and {{true}} that satisfy the constraint.
In other words, {{()~~(trace* id)~~() }}.

%% \label{ch3:ex:annihilate}
Consider another combinator that also has the type {{bool * 1 <-. bool * 1}},
but is instead defined to be {{c = swap+ (*) id }}, which negates the boolean
component of its incoming
...
that {{(b,()) ~~swap+ (*) id~~ (b,())}} which is impossible since {{swap+}} 
...
%% XXX: do we want to cite something for relational programming?
...
Nevertheless, this can still be useful, as it gives us access to
\emph{relational programming},
which uses set-theoretic relations and their
composition to express computational tasks in a declarative way. Their encoding
in {{PiTF}} uses fractional types (and specifically, their ability to encode the
multiplicative trace operator).
%% XXX: this falls kind of flat without some kind of example. do we want to
%% release code with the paper and mention it here?
