...
We can very that the connected wires do indeed give us teh desired
...
be simulatenously decremented and this is not possible if {{p}} is
...
\item We can hide the use the combinator {{omega : b <-> 0}}
  ...
  infinote loop on all inputs and hence would allow us to entirely
  discard one possible branch. In other words, we are explicitely
...
As we did before, we flip inside out the construction of the map and
apply trace to give us the interpreter:
