...
\begin{document}
\title{On the Construction of Isomorphic Interpreters (Journal Version)} 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Isomorphic Interpreters from Logically Reversible Abstract Machines}
\subsection{Simple Bounded Iterator}
...
%subcode{bnf} include main
% Combinators, c = iso | eta | eps | c (;) c | c (*) c | c (+) c
% Combinator Contexts, cc = [] | Fst cc c | Snd c cc 
%                  &|& LeftTimes cc c v | RightTimes c v cc 
%                  &|& LeftPlus cc c | RightPlus c cc 
% Values, v = () | (v, v) | L v | R v
%
% Machine states = <c, v, cc> | {[c, v, cc]} | <c, v, cc>^{dagger} | {[c, v, cc]}^{dagger}
% Start state = <c, v, []> 
% Stop State = {[c, v, []]}
%
...
% {[ c1, v1, LeftTimes cc c2 v2 ]} &|-->& <c2, v2, RightTimes c1 v1 cc> &~~~~~~~~~~~~ rule 10
% {[ c2, v2, RightTimes c1 v1 cc ]} &|-->& {[ c1 (*) c2, (v1, v2), cc ]} &~~~~~~~~~~~~ rule 11
% <eps, R v, cc> &|-->& <eps, L (-v), cc>^{dagger} &~~~~~~~~~~~~ rule 12
% <eps, L (-v), cc> &|-->& <eps, R v, cc>^{dagger} &~~~~~~~~~~~~ rule 13


%subcode{opsem} include main
%! columnStyle = rclr
% <iso, v, cc>^{dagger} &<--|& {[iso, iso(v), cc]}^{dagger} &~~~~~~~~~~~~ rule 1'
% <c1(;)c2, v, cc>^{dagger} &<--|& <c1, v, Fst cc c2>^{dagger} &~~~~~~~~~~~~ rule 2'
% {[c1, v, Fst cc c2]}^{dagger} &<--|& <c2, v, Snd c1 cc>^{dagger} &~~~~~~~~~~~~ rule 3'
% {[c2, v, Snd c1 cc]}^{dagger} &<--|& {[ c1(;)c2, v, cc ]}^{dagger} &~~~~~~~~~~~~ rule 4'
% <c1(+)c2, L v, cc>^{dagger} &<--|& <c1, v, LeftPlus cc c2>^{dagger} &~~~~~~~~~~~~ rule 5'
% {[ c1, v, LeftPlus cc c2 ]}^{dagger} &<--|& {[c1 (+) c2, L v, cc ]}^{dagger} &~~~~~~~~~~~~ rule 6'
% <c1(+)c2, R v, cc>^{dagger} &<--|& <c2, v, RightPlus c1 cc>^{dagger} &~~~~~~~~~~~~ rule 7'
% {[ c2, v, RightPlus c1 cc ]}^{dagger} &<--|& {[c1 (+) c2, R v, cc ]}^{dagger} &~~~~~~~~~~~~ rule 8'
% <c1(*)c2, (v1, v2), cc>^{dagger} &<--|& <c1, v1, LeftTimes cc c2 v2>^{dagger} &~~~~~~~~~~~~ rule 9'
% {[ c1, v1, LeftTimes cc c2 v2 ]}^{dagger} &<--|& <c2, v2, RightTimes c1 v1 cc>^{dagger} &~~~~~~~~~~~~ rule 10'
% {[ c2, v2, RightTimes c1 v1 cc ]}^{dagger} &<--|& {[ c1 (*) c2, (v1, v2), cc ]}^{dagger} &~~~~~~~~~~~~ rule 11'
% {[eps, L (-v), cc]}  &<--|& {[eps, R v, cc]}^{dagger}  &~~~~~~~~~~~~ rule 12'
% {[eps, R v, cc]} &<--|&  {[eps, L (-v), cc]}^{dagger} &~~~~~~~~~~~~ rule 13'
...
%% % v = (\x.e, rho)
%% %
%% % Machine states = <e, rho, E>
%% % Start state = <e, [], []>
%% % Stop state = <v, rho, []>


%% %subcode{opsem} include main
%% % <e1 e2, rho, E > &|-->& <e1, rho, (rho, e2):E >
%% % <\x.e, rho, (rho', e'):E > &|-->& <e', rho', (\x.e,rho):E >
%% % <\x.e, rho, (\x'.e',rho'):E > &|-->& <e', x' =(\x.e,rho):rho', E >


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Generating Inductive Walks for Algebraic Data Types}
...
(Maybe this section should be a seperate paper altogether)

This section introduces another class of isomorphic interpreters
{{langRevEE}} that arise from algebraic manipulations. Recursive type
definitions are viewed as algebraic polynomials characterizing the
shape of the type. Thus binary trees {{@@x.1+x*x}} are represented as
the polynomial {{x=1+x^2}} and hence {{x^2-x+1=0}}. 


TODO: find a few more algebraic manipulations that give useful
combinators. Depending on which language we are working in, there may
be many. Suggestions:
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Deriving the {{just : a <-> a + 1}} combinator}

In the spirit of the above discussion, the type {{nat}} defined by
{{@@x.1+x}} induces the polynomial {{nat = 1 + nat}}. Consider the
...
{{a + nat = a + nat + 1}} because we know that {{nat = nat + 1}}. 

{{a = a + 1}} by cancelling nat. 

\noindent
These algebraic manipulations can be treated as a computation in
{{langRevT}} by witnessing each transformation with an appropriate
combinator. 

\begin{itemize}
\item Thus we start with the the line corresponding to {{a=a}}:

DIAGRAM

\item And then we construct {{a * nat = a * nat}} by parallel
  composing with {{nat = nat}}.

DIAGRAM

\item And then we {{unfold}} on the RHS

DIAGRAM

\item And finally we use {{trace}} to do the appropriate cancellation,
  thereby getting the final circcuit, which we call {{just}}:
...
\noindent
Computationally the {{langRevT}} circuit does the following:

\begin{enumerate}
\item {{forall (v:a).just v |--> right v}} 
\item {{forall (v:a).just^{dagger} (right v) |--> v}} 
\item {{just (left ())}} diverges. 
\end{enumerate}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Isomorphisms of Trees}

Algebraic manipulations like the above have questionable mathematical
validitiy induced by the fact that the polynomial {{nat = nat + 1}} is
not well defined. Hence axiomatically adding such a polynomial may be
suspected as the underlying reason for the divergence. 


In this context let us examine the isomorphism from one binary tree to
seven binary trees that is examined in the Fiore's paper
\cite{Fiore:2004}. Binary trees imply the polynomial {{t^2-t+1=0}}
which does have a valid root, namely {{t = 1/2 + i*sqrt(3)/2}}. 

ALGEBRA

etc. 

Which gives us the circuit: 

DIAGRAM

Indeed this circuit is proof of the embedding of trees in a strong
constructive sense i.e. we can supply a binary tree and examine the
resulting seven binary trees. Further the adjoint of the circuit
allows us to supply seven binary trees and get one resultant tree. 

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Non-constructive Isomorphism of Trees}
However not all valid algebraic manipulations of a binary trees also
have meanignful computational interpretations. Consider the same
isomophism explained with the algebraic manipulation:
...
The construction underlying this isomorphism is {{t^3 = -1}}, which
can be expressed in {{langRevEE}} as the cicruit:

\begin{center}
\scalebox{1.0}{
  \includegraphics{diagrams/fiore_t3.pdf}
}
\end{center}
