...
\item Slide the two sections to expose the start and stop state and
  ...
  mechanical process completely independant of the logic of the machine.
\end{enumerate}
...
\item \emph{Use better types.}  One could augment {{langRevT}} with a
  ...
  entirely. Similarly, in the meta-circular intepreter in
...
\item The definition here shows only the handling of the composition
  ...
  {{rule~1}}. This should be read as `tranform {{v}} according to the
  primitive isomorphism {{iso}}'. Implementing this is straightformard
  and is hence skipped.

\item Stuck states in this machine correspond to runtime values not
  matching their expected types. They can be handled as decribed in
  Sec. \ref{sec:adder} using GADTs (which eliminates them entirely),
  divergence, or adding extra halting states. We have abtracted from
  this choice and marked the relevant cases with combinators labelled
  `{{cast}}'.
...
of {{langRevT}} programs. We have demonstarted the effectiveness of
