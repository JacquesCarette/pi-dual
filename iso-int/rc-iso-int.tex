...
we're not interested in this; we want to directly write partial isomorphisms
 ...
 - design reversible abstract machines by hand (e.g.,
 SE(M)CD~\cite{Kluge:1999:SEMCD} or SECD-H~\cite{lorenz}) and then implement
 them; this is what we are doing; our implementation is in this language in
 which every computation is a partial isomorphism so the implementation will
 be checked to be reversible by construction

 - generally one can take any reversible algorithm; transform it to a
 small-step reversible abstract machine, and then implement the machine in
 our
 language~\cite{Danvy:2008:RRN:1813347.1813350,Rendel:2010:ISD:1863523.1863525}.

 \cite{Yokoyama:2008:PRP,Axelsen:2011:RPC:1987171.1987176} argue that the
 clean simulation of reversible Turing machines is a criterion for computing
 strength of reversible languages. This is kind of what we show.

 \cite{Yokoyama:2007:RPL:1244381.1244404} also show a reversible
 self-interpreter which is what we show

 we argue and so does \cite{Rendel:2010:ISD:1863523.1863525} that partial iso
 are a good foundation for reversible programming
...
\item \todo{fold, unfold, and trace}
\end{itemize}
