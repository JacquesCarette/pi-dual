...
Say we wished to build a {{langRevT}} combinator to map {{nat}} to a
parity encoded version {{par = @@x.1+(1+x)}}. Since {{par}} is not a
fixed unfolding of {{nat}}, it is not immediately apparent how such a
...
\begin{center}
\scalebox{1.0}{
  \includegraphics{diagrams/nat-parity.pdf}
}
\end{center}
