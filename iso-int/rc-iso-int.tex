...
\item The {{trace+}} operation is drawn as a looped circuit, where the
  traced type {{b1}} is shown as flowing backward. 
%% The semantics and
...
{{left ()}} and {{right()}} as {{true}} and {{false}}. Previous
work\cite{James:2012:IE:2103656.2103667} (see Sec 5) also establishes
that {{langRevT}} is Turing complete by showing the compilation of a
Turing complete language -- a first-order language with numbers and
...
type {{1}}. If the number was non-zero, the output is on the bottom branch
...
realized so far. We do this by first ``sliding'' the two sections of the
...
\noindent
we exchange sections A and B while maintaining the connectivity of the wires:
...
This rearrangement exposes that the middle connections were really recursive
calls to the machine transition. The final figure is given below: 
...
\paragraph*{Input Machines.}

Our translation apply to machines presented as small-step operational
semantics, i.e. machines that are step-wise rewrites of tuples of
algebraic datatypes, wherein each rewrite step must be logically
reversible. The formal criterion logical reversibility in this setting
are captured in Abramsky's presentation of \emph{bi-orthogonal
  automata} \cite{abramsky2005structural} and may be summarized as:
\begin{enumerate}
\item
...
\item Each valid machine state must match a unique pattern on the
  left-hand-side and right-hand-side of the `{{|-->}}' rewrite.
...
\paragraph*{Steps of the Translation.}

Although trivial, the previous example captures the fundamental steps
of our construction. For reference, these steps are:
\begin{enumerate}
...
%subcode{opsem} include main
%! columnStyle = rclr
% <true, L n, c> &|-->& <false, L (n+1), c> &~~~~~ rule 1
% <true, N t1 t2, c> &|-->& <true, t1, Lft c t2> & rule 2 
% <false, t1, Lft c t2> &|-->& <true, t2, Rgt t1 c> & rule 3
% <false, t2, Rgt t1 c> &|-->& <false, c, N t1 t2> & rule 4
\end{multicols}
\end{small}
...
On the input side, for {{true}} states, we have expanded the tree component and
for {{false}}~states we have expanded the tree contexts. Similarly we have done
...
\item The definition here shows only the handling of the composition
  ...
  {{iso}}.'' Implementing this is straightforward and is hence skipped.
