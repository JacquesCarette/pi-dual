...
\item Operations {{fold}} and {{unfold}} are specific to the recursive
  data type and are drawn as triangular boxes. Sec. \ref{sec:bounded}
  for instance shows the {{fold}}/{{unfold}} for recursive type of
  natural numbers, {{nat=@@x.(1+x)}}.

\item The {{trace+}} operation is drawn as a looped circuit, where the
  traced value is shown as flowing backward. The semantics and
  interpretation of {{trace}} are dealt with in detail in previous
  work (see Sec. 4 of \cite{infeffects}).
...
In a more general sense, all abstract machines have to deal with stuch
states and there are several standard ways of delaing with them:
\begin{itemize}
\item \emph{Use better types.} One could augment {{langRevT}} with a
  ...
  that can be zero, thus emilinating this situation entirely. In the
  intepreter in Sec. \ref{sec:langrevt-int} for instance one could use
  a GADT to eliminate stuck states.

\item \emph{Diverge.} Another standard approach in dealing with stuck
  states is to make the diverge or leave the output
  undefined/unobservable in some way. For instance {{add1}} defined in
  {{langRevT}} (see Sec. 4.2 in \cite{James:2012:IE:2103656.2103667})
  has teh property that {{add1^{dagger} 0}} diverges.

\item \emph{Stop the machine.} In the case of our machine this
  corresponds to another possible kind of start state {{<n,0,n>}} to
  make the isomorphism total.  Let us choose this path in the case of
  this machine.  After expanding the input to expose $n$ and the
  output to expose both $p$ and $m$, we wire the machine corresponding
...
Note that there are indeed two start states in the interpreter. We can
...
In the final construction we present a metacircular interpreter for
...
easier to compare with the abstract machine.  A couple of important
points:

\begin{enumerate}

\item The definition here shows only the composition combinators;
  which are the intersting cases. All the primitive isomorphisms are
  hidden in the `{{iso(v)}}' application in {{rule 1}}. This should be
  read as `tranformation {{v}} according to the isomorphism
  {{iso}}'. Implementing this is straightformard and is hence skipped.

\item Stuck states in this machine correspond to values not matching
  the shape expected by their types. They can be handled as decribed
  in Sec. \ref{sec:adder} using GADTs (which eliminates them
  entirely), divergence, or adding extra halting states.
