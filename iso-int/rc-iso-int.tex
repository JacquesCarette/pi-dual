...
\item Operations {{fold}} and {{unfold}} are specific to the recursive
...
\item The {{trace+}} operation is drawn as a looped circuit, where the
  traced type {{b1}} is shown as flowing backward. The semantics and
...
Our goal is to implement this abstract machine in {{langRevT}}: We
start by writing a combinator {{c : nat * nat <-> nat * nat}} that
executes a `small step'. Here {{nat}} is an abbreviation for the
recursive type {{@@x.1+x}}. The combinator, when iterated, should map
{{(n,0)}} to {{(0,n)}} by precisely mimicking the steps of the
abstract machine. Let us analyze this desired combinator {{c}} in
detail starting from its interface:
...
start the machine and the exposed stop state returns any result values
...
\item Slide the two sections to expose the start and stop state and
  introduce a {{trace}} to iterate the construction. This is always a
  mechanical process completely independent of the logic of the machine.
\end{enumerate}
...
machine gets stuck at a state that is not a valid start state.

\paragraph*{Stuck States.}
All abstract machines, not just those written in {{langRevT}}, have to
deal with stuck states. There are several standard ways of dealing
with them:
\begin{itemize}
\item \emph{Use better types.}  One could augment {{langRevT}} with a
  richer type system that distinguishes non-zero numbers from those
  that can be zero, thereby eliminating the {{sub1 0}} situation
  entirely. Similarly, in the meta-circular interpreter in
  Sec. \ref{sec:langrevt-int} one could use GADTs to eliminate stuck
  states.

\item \emph{Divergence.}  Another standard approach in dealing with
  stuck states is to make the machine diverge or leave the output
  undefined/unobservable in some way. The {{add1}} combinator defined
  in Sec. 4.2 of our related work \cite{James:2012:IE:2103656.2103667}
  has the property that its dual {{sub1}} diverges on input 0. Hence
  we could just use {{add1}} here.

\item \emph{Stop the machine.}  If we chose to stop the machine at a
  stuck state, we are essentially treating states of the form
  {{<n,0,n>}} as valid stop states for reverse execution. This also
  has the consequence of making our isomorphism total and also gives
  us two valid start states in the case of forward execution. If we
  choose this approach, the machine would look as shown below by the
  end of {{step 3}}:
...
sub-tree is currently being explored. The definitions are shown on the left:
...
also switching focus to any unexplored sub-trees in the process. There
...
states simply hide an implicit {{bool=T~|~F}}. We make this explicit:
...
isomorphic tree-traversal interpreter, except for the boring final
step. (Partially because of the implicit non-termination introduced by
the {{add1}}).
...
machine with several cases, but the deriving the corresponding
{{langRevT}} interpreter follows in exactly the same way as
before. Here is a logically reversible small-step abstract machine for
{{langRevT}} and the derived isomorphic interpreter with the
reductions labeled.
...
\item The definition here shows only the handling of the composition
  ...
  isomorphisms are hidden in the `{{iso(v)}}' application in
  {{rule~1}}. This should be read as `transform {{v}} according to the
  primitive isomorphism {{iso}}'. Implementing this is straightforward
...
We have developed a technique for the systematic development of
{{langRevT}} programs from logically reversible small-step abstract
machines. Since techniques for developing small-step interpreters are
well developed this allows for the direct development of a large class
of {{langRevT}} programs. We have demonstrated the effectiveness of
this approach by deriving meta-circular interpreter for {{langRevT}}
in this style.
