...
\item The abstract machine definition is not very strongly typed in
  teh following sense: we know that every {{v:b1*b2}} must have the
  form {{(v1,v2)}} however is cases like {{rule 8}} if we allow any
  {{v}} to appear the machine will be in an undefined state. The usual
  way this is handled in any interpreter is to either test to ensure
  that the runtime value has teh required form or to user s stronger
  typing discipline such as GADTs.We abstract away from this choice in
  this presentation.
\end{enumerate}
