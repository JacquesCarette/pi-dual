...
$\nodet{S}{\emptyset}$ which is essentially the denotation of conventional
first-order types. As we explain below, cubical sets are closed under sums,
differences, products.

Examples of 1-dimensional cubical sets include all the sets of the form
$\nodet{S}{\emptyset}$ where $S$ is finite set denoting a regular first-order
types $t$. A 2-dimensional cubical set looks like $\nodet{S_1}{S_2}$ which
intuitively corresponds to the difference $t_1 - t_2$ of the two types whose
denotations are $S_1$ and $S_2$ respectively. A 3-dimensional cubical type
...
\[\begin{array}{rcl}
\denc{t} &=& \nodet{\den{t}}{\emptyset} = 
  \nodet{\nodet{\den{t}}{\emptyset}}{\emptyset} = \ldots \\
