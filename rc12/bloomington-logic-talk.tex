...
The \lcal has information effects.  Can we have a language with
binders that does not entail information effects?  \vfill


\vfill
Can we design a high-level language out of what we just saw?
...
%type Nat = 0 | Succ Nat
%
%type Tree = Node Tree Tree | Leaf Nat
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]{Theseus: Reversing Maps}
...
%rev_treeUnwind :: Tree * Tree + Nat <-> Tree
...
\pause
  ...
  \begin{enumerate}
  \item Labels must be typed. 
  \item Somewhat like Agda views. 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]{Theseus : Tree walking}
...
%! language=haskell
%! style = bluescript
%! morekeywords = {Nat,Tree,Ctxt,reconst}
%type Ctxt = Empty
%          | L Ctxt Tree
%          | R Tree Ctxt
%
%treeWalk :: Tree <-> Tree
%| tr                       <-> walk $ tr, Empty
%| walk $ Leaf b, ctxt      <-> reconst $ ctxt, Leaf (not b) 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]{Theseus : An important map}
...
%just :: a <-> a + 1
%| a                    <-> iter $ Right a 
%| iter $ Right a       <-> Right a 
%| iter $ Left 0        <-> Left ()
%| iter $ Left (Succ n) <-> iter $ Left n 
% where iter :: Nat + a
...
%negate :: f:(a <-> b) -> -a <-> -b 
%| na               <-> u1 $ Left na 
%| u1 $ a_zero      <-> cancel a_zero
%| u2 $ Right (f a) <-> u1 $ Right a 
%| u2 $ Left nb     <-> nb 
%| cancel b_zero    <-> u2 $ b_zero
%where u1 :: -a + a 
%      u2 :: -b + b
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]{Theseus : More}

\vfill
\begin{scriptsize}
  Definition {{a -o b}} : Is a shorthand for {{(-a + b)}}
\end{scriptsize}
...
\vfill
  \item Preserve variables across the {{<->}} boundary.
\vfill
  \item Types of constructors don't matter. Types of constants don't matter.
\vfill
  \item Maps can be reversed.
\vfill
  \item Monomorphised.
\vfill
  \item All the {{->}} are inlined to {{a <-> b}} at compile time. 
\vfill
  \item Typed labels for iteration (approximate Agda views). 
\vfill
  \item {{Pi0}} primitives can be built.
\vfill
  \item Can be translated to {{Pi0}}.
\vfill
  \end{enumerate}
...
\vfill
  \item Build a compiler.

\vfill
  \item Interesting thing is the type checker.

\vfill
  \item Open: can we make the type system richer? 
    \begin{enumerate}
    \item Bounded types, universes, polymorphisms, existentials -- suspect yes to most. 
    \end{enumerate}

\vfill
  \item Proofs: Connection to dagger symmetric traced bimonoidal categories. 

\vfill
  \item Information effects and type-and-effect system.

\vfill
  \item Can we have an implicit runtime stack and support for inductive
    walks without defining a context type?

\vfill
  \item Can we have higher order things, first class patterns, dual
    objects and continuations?
