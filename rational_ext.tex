...
\begin{lemma}[Dagger]
  ...
  \item {{(f (;) g)^{dagger} = f^{dagger} (;) g^{dagger}: b1 <-> b2}}
...
\begin{lemma}[Symmetric Monoidal (+, 0)]
  ...
  \begin{proof}
    ...
    \item The braiding is self inverse {{swap+_{b1+b2} = (swap+_{b2+b1})^{dagger} }}.
    \end{enumerate}
...
\begin{lemma}[Symmetric Monoidal (*, 1)]
  ...
  unit {{1}}. The details here follow those of the {{(0, +)}} monoid
  and are skipped.
\end{lemma}
...
\item The category {{Pi}} has no initial and terminal objects. The
  ...
  all functions (is in the category {{CatSet}}).

\item The category {{Pi}} has neither categorical products, nor
  categorical co-products -- {{*}} and {{+}} are merely monoidal
...
is concievable that part of its structure collapses. The sort of
collapse, while catastrophic for algebraic structures (effectively
trivializing them) still retains some interest in computing, because in
computing we are interested in the specific operational nature (the
computation content, so to speak) of the morphisms. Anectodal evidence
