...
isomorphisms.  They have (iso) types {{v \rightarrow v}}.  
...
\footnote{where {{swap*}} and {{swap+}} are self-dual}
...
It is worthwhile to remember that the above correspond to the \emph{axioms}
of commutative semirings.  However, seen as universally quantified,
orientable statements, these axioms also induce transformations of the
(traditional) values. \jacques{include code here to be explicit}
...
combinators of {{Pi}} to the world of vector spaces. Intuitively:
\begin{itemize}
\item instead of interpreting a type {{b}} as a set of cardinality
  $\modulusp{b}$, we now interpret each type {{b}} as a vector space
  $\vecspace{b}$ of dimension $\modulusp{b}$ over the field of booleans
  $\Fpx{2}$;
\item the elements of such a vector space are column vectors whose elements
  are 0 or 1 and which indexed by the values of type {{b}}, i.e., 
  a value {{v_i : b}} is interpreted as the vector:
\[\begin{array}{r@{~}l}
\begin{array}{r} 
v_0 \\ v_1 \\ \vdots \\ v_i \\ \vdots \\ v_n 
\end{array} & 
\begin{bmatrix}
0 \\ 0 \\ \vdots \\ 1 \\ \vdots \\ 0
\end{bmatrix}
\end{array}\]
\item each combinator {{c : b1 <-> b2}} is interpreted as a matrix whose
  columns are indexed by the values of type {{b1}} and whose rows are indexed
  by the values of type {{b2}}. The matrix is \emph{orthogonal} with each row
  and column containing exactly one occurrence of the value 1.
\end{itemize}

Now that the types are interpreted as vector spaces, it is possible to add a
new type {{1/b}} which corresponds to the \emph{dual} of the vector space
denoting {{b}}. The elements of this dual vector space are \emph{linear
  functionals} mapping vectors in $V_b$ to a scalar (0 or 1). The isomorphism
between the types {{1}} and {{b * 1/b}} is now a consequence of that fact
that the pairing of a functional in the dual space $\vecspace{1/b}$ and a
vector in the space $\vecspace{b}$ produces a scalar.

The alert co-author should have noticed that using $\Fpx{2}$ as the underlying
field is actually nonsense. To get things to work properly we need a field of
one element, which apparently exists and has some nice properties. 
...
%subcode{bnf} include main
% value types, b ::= 0 | 1 | b + b | b * b | S b
% values, v ::= () | left v | right v | (v, v) | {| v1, v2, dots |}
...
\begin{definition}[Size of a type]
The defintion of size is extended to include negative types.
{{[^-b^] = -[^b^]}}
\end{definition}
