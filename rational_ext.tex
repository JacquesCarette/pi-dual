...
For any execution path from {{v}} to {{v'}} in {{c}}, {{c{dagger} }}
has an execution path from {{v'}} to {{v}}.  Stated differently,
logical reversibility implies that {{c^{dagger} }} denotes the inverse
relation that {{c}} denotes, i.e.  {{c^^^={|(v',v)~|~(v,v') elem c|} }}.  
Intuitively logical reversibility holds because for every
{{eta*}} reduction in a world that transforms {{()}} into {{(1/v,v)}},
on reverse execution the adjoint {{eps*}} will transform {{(1/v,v)}}
back to {{()}}.
