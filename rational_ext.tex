...
Given a combinator {{c:b1<->b2}} and a value {{v:b1}} we can define
...
The definition of equality between combinators has to be extended to
reflect the fact that combinators can have multiple outputs for an
input. More generally, combinators denotes relations. 

\begin{definition}[c1 = c2]
We say that {{c1 = c2}}, for combinators {{c1:b1<->b2}} and
{{c2:b1<->b2}} iff the denote the same relation between sets {{b1}}
and {{b2}}. Equivalently,
...
The logical reversibility is extended to relations and should be
understood as follows. The combinator {{c:b1<->b2}} denotes a relation
from {{b1}} to {{b2}} and its adjoint denotes the inverse relation
from {{b2}} to {{b1}}. Operationally, for any execution path from
{{v}} to {{v'}} in {{c}}, {{c{dagger} }} has an execution path from
{{v'}} to {{v}}.

\roshan{This is an important question -- is {{PiEE*}} still
  bimonoidal? i.e. so all the coherence conditions on isomorphisms
