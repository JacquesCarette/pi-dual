...
\begin{abstract}
  ...
  \cite{infeffects,rc2011,rc2012} and the core calculus for computing
  with the isomorphisms of finite types, {{langRev}}. Given the
  first-order nature of {{langRev}} however it was unclear how one may
  express first-class functions.  Andrej Filinski's masters thesis
  ...
  systemmatically from the {{langRev}} by considering a symmetric
  notion of duality. Unlike the \lcal which shows a classical demorgan
  dulaity, here we have two axis of dualization and hence two
  dualities -- an additive duality, namely negative types and a
  multiplicative duality, namely fractional types.
...
Andrej Filinksi's masters thesis
\cite{Filinski:1989:DCI:648332.755574} suggested a remarkable symmetry
underlying computation is one embraces the symmetry hidden in
continuations. Filinski showed that values are dual to continuations,
functions are dual to delimited continuations. Filinksi also shows a
de-morgan style duality, by treating value pairs as the dual of
continuation sums and value sums as dual to continuation pairs.
...
{{langRev}} computation always terminate.  
...
It is useful to contrast the categorical structure of {{langRevEE}},
which has two distinct \emph{compact closed} structures, to the
well-known and closed related \emph{*-autonomous} categories that form
a model for linear logic. The distnguishing feature that seperates
*-autonomous structures from compact closed structures is that the
dualizing action in *-autonomous categories does not preserve the
tensor. In {{langRev}}, as we will see , we can construct the
isomorphism {{-(b1+b2)<-> (-b1)+(-b2)}}, i.e. the tensor {{+}} has
been preserved by the negation.  Whereas in linear logic we have that
$(a\oplus b)^\bot$ is $a^\bot \parr b^\bot$, i.e. the connective
$\oplus$ is not preserved. 

In linear logic terms `additive pairs' and `multiplicative sums' are
used to indicate this notion that some connectives has both additive
and multiplicative properties. The usual intuitive interpretations for
these involve suggestions like one has a dollar to spend and the both
cigarettes and soda are available for purchase, but the fact that you
have only a dollar to spend affords you only one of these. The
intuitions talk about a sort of dependacy underlying the fact that
both cigarettes and soda are available but you can have only one. 

In {{langRevEE}}, operators {{eta}} and {{eps}} capture a similar
notion.The pair of values {{v}} and {{1/v}} created by {{eta*}} are
not fully independant in the way one would think of normal pairs. The
value of one, affects the value of the other. Similarly, the {{v}} and
{{-v}} created by {{eta+}} do really present a strict choice of
{{left}} or {{right}} in the sense that usual sums do, but instead can
have a \emph{both} {{left}} and {{right}} values at different points
during the computation. However, in our opinion, the philisophical
difference the additiveness of the additive pairs and the
multiplicativesness of the multiplicative pair aren't a property of
the connective between the values, but a property intrinsic to the
values themselves. Hence {{v}} and {{1/v}} share secretely a
relationship that their combinting {{*}} constructor does not and need
not know about. 

\roshan{We need to prove this. I am not sure how to start.}
\begin{proposition}[Strong Normalizing]
\label{prop:termination-piee-plus} 
Computations in {{langRev}} extended with {{eta+}} and {{eps+}} are
always terminating.
\end{proposition}


\roshan{This is straightforward.}
\begin{proposition}[Strong Normalizing]
\label{prop:termination-piee-times} 
Computations in {{langRev}} extended with {{eta*}} and {{eps*}} are
always terminating.
\end{proposition}


\begin{proposition}[Weak Normalizing]
\label{prop:termination-piee} 
Computations in full {{langRevEE}} are weak normalizing.
\end{proposition}

\begin{center}
  \includegraphics{diagrams/inf1.pdf}
\end{center}

\subsection{Logical Reversibility}

\begin{proposition}[Logical Reversibility]
{{langRev}} with {{eta+}} and {{eps+}} is logically reversible.
\end{proposition}
\roshan{Proof follows from compilation to {{langRevT}} }

\begin{proposition}[Logical Reversibility]
{{langRev}} with {{eta*}} and {{eps*}} is logically reversible.
\end{proposition}

This is an awkward one. I dont thnk that the above statement is
right.. At least not in the usualy sense. We can construct the
...
{{c1{dagger} true |--> {[true, false ]} }}

{{c1{dagger} false |--> {[true, false ]} }}
...
{{c2{dagger} false |--> true }}

Probably the right way to approach this is to see what they do in
physics. If fractionals do correspond to measurement and the
categorical structures of Coecke and Abramsky, then the ideas of
reversibility in that setting should also apply here.

I don't think this issue will go away by any sort of choice of
top-level. I tried to design a top-level that can fix this -- I dont
