...
worlds, each of which is seeded by a unique value {{v}} that inhabits
the type {{b}} that indexes {{eta* : 1 <-> 1/b * b}}. Each world has
an independant existence. As an example at the type {{bool}}, we would
have two possible machine states:
...
In the discourse below we use {{c v |-->* v'}} as a shorthand for the
execution sequence {{<c,v,[]>|-->*{[c,v',[] ]} }}.
...
The above definition implies that combinators {{c:b1<->b2}} can be
indetified by relations between the sets {{b1}} and {{b2}}, i.e. 
{{c:b1<->b2 = {| (v, v') | v:b2, v':b2 and c v |-->* v'|} }}.
The definition of equality between combinators is extended reflect the
fact that combinators denote relations.

\begin{definition}[c1 = c2]
We say that {{c1 = c2}}, for combinators {{c1:b1<->b2}} and
{{c2:b1<->b2}} iff they denote the same relation between sets {{b1}}
and {{b2}}. Equivalently,
...
Logical reversibility of {{PiEE*}} implies that {{c^{dagger} }}
denotes the inverse relation that {{c}} denotes, i.e.
{{c^^^={|(v',v)~|~(v,v') elem c|} }}.  Operationally, for any
execution path from {{v}} to {{v'}} in {{c}}, {{c{dagger} }} has an
execution path from {{v'}} to {{v}}.  Intuitively, logical
reversibility holds because for every {{eta*}} reduction in a world
that transforms {{()}} into {{(1/v,v)}}, on reverse execution the
adjoint {{eps*}} will transform {{(1/v,v)}} back to {{()}}.
...
Like {{Pi}}, {{PiEE*}} is a commutative bimonoidal catgeory.
Symmetric monoidal categories where every object had a dual object and
where dualizing preserves the monodal tensor are referred to as
compact closed categories. Since divison is defined only for types of
non-zero size, not all objects have dual objects.  In this section
establish that {{PiEE*}} is ``locally compact closed'' in the sense
that for non-zero objects, namely types where {{[^ b ^]=0}}, it
satisfies the conditions for compact closed categories.
