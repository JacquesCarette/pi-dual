...
combinators of {{Pi}} to the world of \emph{pointed sets}. Intuitively:
\begin{itemize}
\item instead of interpreting a type {{b}} as a plain set of cardinality
  $\modulusp{b}$, we now interpret each type {{b}} as a pointed set $P_b$
  with a distinguished \emph{basepoint} $\bot_b$;
\item a combinator {{c : b1 <-> b2}} is interpreted as a function as before
  with the restriction that it must map the basepoint $\bot_{b_1}$ to the
  basepoint $\bot_{b_2}$.
\end{itemize}

Intuitively a pointed set may be seen as a vector space over the field with
one element. This view allows us to add the type {{1/b}} and interpret it as
the \emph{dual} of the vector space denoting {{b}}. Concretely the elements
of the pointed set denoted by {{1/b}} are \emph{linear functionals} $f : P_b
\rightarrow P_1$. Note that $P_1$ is the pointed set containing {{()}} and
$\bot_1$. The isomorphism between the types {{1}} and {{b * 1/b}} is now a
consequence of that fact that the pairing of a vector and a functional in the
dual space produces a scalar. 
