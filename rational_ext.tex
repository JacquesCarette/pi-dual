...
\begin{itemize}
\item each type {{b}} is now interpreted as a vector space of dimension
  $\modulusp{b}$ over the field $\Fpx{2}$; the elements of such a vector
  space are column vectors
\item each value {{v_i : b}} is interpreted as column vector:
\[\begin{array}{r@{~}l}
\begin{array}{r} 
...
Once we are in the world of vector spaces, the fractional type {{1/b}} is
interpreted as the \emph{dual} to the vector space denoted by {{b}}. The
elements of this dual vector space can be represented as \emph{row vectors}. 

Multiplying a row vector by a column gives a scalar: this is eta and epsilon.
