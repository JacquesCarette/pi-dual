...
$b_2$. Instead of presenting such a semantics, we present a slightly more
involved semantics using discrete groupoids. In that semantics, the
denotation of each type will consist of (i) a pointed set, and (ii) a trivial
equivalence relation relating each element to itself by a \emph{path}. The
pointed sets are constructed by adjoining, to each ``regular'' set, an
element $\bot$ with a non-trivial path to itself which, topologically,
corresponds to the \emph{circle}. Significantly, using the Euler
characteristic as a generalized notion of cardinality, a set containing $n$
discrete elements (with only trivial reflexivity paths) and a circle has
cardinality $n$ and not $n+1$.\footnote{Generally speaking, this generalized
  cardinality based on the Euler characteristic counts the number of
  ``connected components'' of a set. Consider a set with $n$ points and
  various paths connecting them. We start by counting all the points giving
  us cardinality $n$. Then for every path, we decrease the cardinality by one
  to avoid double-counting connected elements. Then we add one again for
  every path that forms a loop because the path forming the loop did not
  decrease the number of connected components. It is easy to see that a point
  with a path that connects it to itself would have generalized cardinality
  0. Moreover the generalized cardinality of the disjoint union of two sets
  is their sum and the generalized cardinality of the product of two sets is
  their product.}

\begin{definition}[Denotation of types]
Each type $b$ is mapped to a groupoid $(\dt{b}, \bot_b, \mathit{paths}_b)$
where~$\dt{b}$ is a pointed set with distinguished point $\bot_b$ and
$\mathit{paths}_b$ is a collection of paths, $v_1 \path v_2$,
between elements $v_1$ and $v_2$ of $\dt{b}$. (We omit the trivial paths for
reflexivity and only include the non-trivial ``generating'' paths as
suggested by the notation of \emph{higher-order inductive types}.)
\[\begin{array}{rcl@{\qquad}l}
\dt{0} &=& (\{ \bot_0 \}, \bot_0, \{ \bot_0 \path \bot_0 \}) &
           (\mbox{circle}) \\
\dt{1} &=& (\{ \bullet, \bot_1 \}, \bot_1, \{ \bot_1 \path \bot_1 \}) &\\
\dt{(b_1 + b_2)} &=& \dt{b_1} \wedgesum \dt{b_2} & 
           (\mbox{wedge~sum}) \\
\dt{(b_1 * b_2)} &=& \dt{b_1} \smashproduct \dt{b_2} & 
           (\mbox{smash~product})
\end{array}\]
The empty type is mapped to the topological circle which is a point that is
connected to itself. The unit type is mapped to a pointed singleton with a
distinguished element connected to itself with a non-trivial loop. The
operation $\wedgesum$ is the \emph{wedge sum} which corresponds to the
disjoint union with an additional path $\leftv{\bot_{b_1}} \path
\rightv{\bot_{b_2}}$ identifying the two distinguished elements of the
component spaces. The new cluster is the distinguished element of the
composite space. The operation $\smashproduct$ is the \emph{smash product}
which is the regular set product with additional paths connecting
$(x,\bot_{b_2})$ and $(\bot_{b_1},y)$ for all $x \in \dt{b_1}$ and $y \in
\dt{b_2}$. The new cluster of identified distinguished elements is the new
distinguished element of the composite space.
\end{definition}

We quickly check that the desired isomorphisms hold at the level of types,
e.g., the isomorphism $b * 0 \iso 0$ requires that the smash product of any
groupoid with the circle be indistinguishable from the circle.
