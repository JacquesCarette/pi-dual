...
$A$ is a directed edge $p$ from $x$ to $y$. The notation $\transport{p}{x}$
denotes the action of \emph{transporting} the point $x$ to the point~$y$
along $p$.
\end{definition}

\begin{definition}[Disjoint union of paths]
Given two sets of paths $P$ and $Q$, the disjoint union $P \uplus^p Q$ is
defined as:
\[ 
  \{ \leftv{p} : \leftv{x} \path \leftv{y} ~|~ (p : x \path y) \in P \}
  \union
  \{ \rightv{q} : \leftv{x} \path \leftv{y} ~|~ (q : x \path y) \in Q \}
\]
\end{definition}
We will sometimes write this as $\leftv{P} \union \rightv{Q}$.

\begin{definition}[Product of paths]
Given two sets of paths $P$ and $Q$, the product $P \times^p Q$ is
defined as:
\[ 
\{ (p,q) : (x,z) \path (y,w) ~|~ (p : x \path y) \in P, (q : z \path w) \in Q \}
\]
\end{definition}
We will sometimes write this as $(P,Q)$.

\begin{definition}[Groupoid]
A \emph{groupoid} $G = (G_0, \mathit{paths}_{G_0})$ is a set $G_0$ together
...
A pointed groupoid $G_{\bullet} = (G_0, \bot_{G_0}, \mathit{paths}_{G_0})$ is
a groupoid $(G_0, \mathit{paths}_{G_0})$ with a distinguished element
$\bot_{G_0} \in G_0$.
\end{definition}
...
\[\begin{array}{l}
(G_0, \bot_{G_0}, \mathit{paths}_{G_0}) \wedgesum 
(H_0, \bot_{H_0}, \mathit{paths}_{H_0}) = \\
((G_0 \uplus H_0) \union \{ \bot \},
 \bot,
 (\mathit{paths}_{G_0} \uplus^p \mathit{paths}_{H_0}) \union
...
follows:
\[\begin{array}{l}
(G_0, \bot_{G_0}, \mathit{paths}_{G_0}) \smashproduct
(H_0, \bot_{H_0}, \mathit{paths}_{H_0}) = \\
((G_0 \times H_0) \union \{ \bot \},
 \bot,
 (\mathit{paths}_{G_0} \times^p \mathit{paths}_{H_0}) \union
...
((\leftv{G_0} \union \{ \rightv{\bot_0}, \bot \}, 
 \bot,
 (\leftv{\mathit{paths}_{G_0}} \union \{ \rightv{\mathit{loop}^n_0} , 
