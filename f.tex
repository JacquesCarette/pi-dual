...
element $\circ$ which topologically corresponds to the
\emph{circle}. Significantly, using the Euler characteristic as a generalized
notion of cardinality, a set containing $n$ ``regular'' elements and a circle
$\circ$ has cardinality $n$ and not $n+1$.\footnote{Generally speaking, this
  generalized cardinality based on the Euler characteristic counts the number
  of ``connected components'' of a set. Consider a set with $n$ points and
...
Each type $b$ is mapped to a groupoid $(\dt{b}, \circ_b, \mathit{paths}_b)$
where $\dt{b}$ is a pointed set with distinguished point $\circ_b$ and
$\mathit{paths}_b$ is a collection of paths, $v_1 \path v_2$, between elements
$v_1$ and $v_2$ of $\dt{b}$. The map is defined as follows:
\[\begin{array}{rcl@{\qquad}l}
\dt{0} &=& (\{ \circ_0 \}, \circ_0, \{ \circ_0 \path \circ_0 \}) &
          (\mbox{circle}) \\
\dt{1} &=& (\{ \bullet, \circ_1 \}, \circ_1, 
           \{ \circ_1 \path \circ_1, \bullet \path \bullet \}) &
          (\mbox{0-sphere})\\
...
connected to itself. The unit type is mapped to the topological 0-sphere
which is a pair of points with the discrete topology. The operation
$\wedgesum$ is the \emph{wedge sum} which is a regular disjoint union with a
new distinguished point $\circ_{b_1+b_2}$ and additional paths consisting of
the equivalence closure of the identifications $\circ_{b_1} \path
\circ_{b_1+b_2}$ and $\circ_{b_2} \path \circ_{b_1+b_2}$. The operation
$\smashproduct$ is the \emph{smash product} which is a regular product with a
new distinguished point $\circ_{b_1*b_2}$ and additional paths consisting of
the equivalence closure of the identifications $(x,\circ_{b_2}) \path
\circ_{b_1*b_2}$ and $(\circ_{b_1},y) \path \circ_{b_1*b_2}$ for all $x \in
\dt{b_1}$ and $y \in \dt{b_2}$.
\end{definition}
